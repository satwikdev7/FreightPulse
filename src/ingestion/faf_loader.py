...
class FAFHiLoLoader:
    """Load the FAF HiLo source CSV into the raw DuckDB layer."""

    table_name = "raw_faf_hilo"

    def __init__(self, csv_path: Path | None = None, db_manager: DuckDBManager | None = None) -> None:
        self.csv_path = Path(csv_path or CONFIG.faf_hilo_csv)
        self.db_manager = db_manager or DuckDBManager()

    def load(self) -> LoadValidation:
        ...
        validation = LoadValidation(self.table_name, expected_rows, actual_rows)
